...
int main(int argc, char **argv) {
  ...
  options.add_options()(
      ...
      cxxopts::value<bool>()->implicit_value("true")->default_value("false"))( //--debug가 있을 경우 true --b,bedug가 없을 경우 false
      "h,help", "Print help");
  ...
  int v4l2lo = 0;
  if (args.count("output")) //default /dev/video1
    v4l2lo =
  ...
  while (cam.is_running) {
    ...
    masked_img = predictor.predict(img);//사람 마스크 예측 처리하고 결과 이미지
    int end_time = cv::getTickCount();
    if (args["debug"].as<bool>())//디버그 옵션이 false면
      std::cout << "Frame process time: "
                << (end_time - start_time) / cv::getTickFrequency()
                << std::endl;
    if (!img.empty()) { //입력이 비어있지않으면 출력한다.
      if (args["debug"].as<bool>()) //if debug option is implicited imshow working
